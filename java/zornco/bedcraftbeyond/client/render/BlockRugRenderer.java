...
public class BlockRugRenderer implements ISimpleBlockRenderingHandler {
	...
	public boolean renderWorldBlock(IBlockAccess world, int x, int y, int z,
		...
		//IIcon textureIndex = block.getBlockTextureFromSide(0);
		...
		for (int i = -1; i < 2; i++) {
			for (int j = -1; j < 2; j++) {
				for (int k = -1; k < 2; k++) {
					...
					else if(ID == BedCraftBeyond.rugBlock)
					{
					...
					else if(BlockStairs.func_150148_a(ID))
					{
	...
	public int stairMainCube(IBlockAccess par1IBlockAccess, int par2, int par3, int par4)
    {
        ...
        if (i1 == 0)
        {
            ID = par1IBlockAccess.getBlock(par2 + 1, par3, par4);
            meta = par1IBlockAccess.getBlockMetadata(par2 + 1, par3, par4);

            if (BlockStairs.func_150148_a(ID) && (l & 4) == (meta & 4))
            {
                dir = meta & 3;
        ...
        {
            ID = par1IBlockAccess.getBlock(par2, par3, par4 + 1);
    ...
    public int stairSecondCube(IBlockAccess par1IBlockAccess, int par2, int par3, int par4)
    {
        ...
        if (i1 == 0)
        {
            ...
            if (BlockStairs.func_150148_a(j1) && (l & 4) == (k1 & 4))
            {
                l1 = k1 & 3;
    ...
    private boolean isBlockStairs(IBlockAccess par1IBlockAccess, int par2, int par3, int par4, int par5)
    {
        ...
        return BlockStairs.func_150148_a(i1) && par1IBlockAccess.getBlockMetadata(par2, par3, par4) == par5;
    }
